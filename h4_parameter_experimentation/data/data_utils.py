...
def get_vocabulary(dataset):
    ...
    ### YOUR CODE GOES HERE
    unique_words = [x[0] for x in dataset]
...
def get_unique_labels(dataset):
    ...
    ### YOUR CODE GOES HERE
    unique_labels = [x[1] for x in dataset]
...
def label_to_index(dataset):
    ...
    # Create a mapping from strings to indices
    ### YOUR CODE GOES HERE
    labels = get_unique_labels(dataset)
    for label in labels:
...
def split_dataset(dataset, word_mapping, label_mapping, percent_testing=0.2, shuffle=False):
    ...
    split_point = int(len(dataset) * (1-percent_testing))
    train_data = dataset[:split_point]
    test_data = dataset[split_point:]
    training_words = [word_mapping[x[0]] for x in train_data]
    training_labels = [label_mapping[x[1]] for x in train_data]
    testing_words = [word_mapping[x[0]] for x in test_data]
    testing_labels = [label_mapping[x[1]] for x in test_data]
    ### YOUR CODE ENDS HERE
