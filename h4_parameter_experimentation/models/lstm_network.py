...
class LSTMNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm = nn.LSTM(embedding_dim,
                            ...
                            batch_first = True)
        output_input = hidden_dim
        if bidirectional:
            output_input *= 2
        self.output_layer = nn.Linear(output_input, output_dim)
        ### YOUR CODE ENDS HERE
    ...
    def forward(self, data_in):
        ...
        lstm_out, (h_, c_) = self.lstm(data_in)
...
def loss(model, loss_fn, x, y, pre_trained, seq_len, device='cpu'):
    ...
    ### YOUR CODE GOES HERE
    if pre_trained:
        data_in = torch.tensor(x, device=device).view(len(x)//seq_len, seq_len, -1)
    else:
        data_in = torch.tensor(x, device=device, dtype=torch.long).view(-1, seq_len)
    labels_out = torch.tensor(y, device=device, dtype=torch.long)
    prediction = model(data_in)
    prediction = prediction.view(labels_out.size(0), -1)
    loss_value = loss_fn(prediction, labels_out)
    
    accuracy = 0.0
    for index in range(len(prediction)):
        if torch.argmax(prediction[index]) == labels_out[index]:            
            accuracy+=1
...
def train_loop(model, optimizer, loss_fn, samples, labels,
    ...
    for index in range(0, len(samples), batch_size*seq_len):
        ### YOUR CODE GOES HERE (delete the continue)
        loss_val,accuracy,num_preds = loss(model, loss_fn, samples[index:index+batch_size*seq_len], labels[index:index+batch_size*seq_len],
                        pre_trained, seq_len, device=device)
        optimizer.zero_grad()
        
        loss_total += loss_val.item()
        acc_total += accuracy
        total_samples += num_preds
        loss_val.backward()
        optimizer.step()
